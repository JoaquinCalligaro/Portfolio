 ...
 */
export function initSlider(sliderId: string | HTMLElement, options: SliderOptions = {}): SliderController | null {
  ...
  try {
    ...
    try {
      if (root.dataset && root.dataset.sliderInitialized === 'true') return null;
    } catch {
      /* noop */
    }
    ...
    const dots = Array.from(root.querySelectorAll('.slider-dot')) as HTMLElement[];
...
try {
  if (typeof window !== 'undefined') {
    (window as typeof window & { initSlider: typeof initSlider }).initSlider = initSlider;
  }
} catch {
  // noop
}
...
export type { SliderOptions, SliderController as SliderControllerType } from './types';
